...
const MINIGAMES = [
  ...
  { id: 'mines', name: 'Mines', desc: 'Find gems, avoid mines — cash out anytime!', free: 5, cost: 25, image: 'mines' },
];
...
// ===== TREASURE HUNT GAME =====
// ========== MINES GAME ==========
function MinesGame({ onClose, onWin }) {
  ...
  const revealTile = useCallback((idx) => {
    ...
    if (mineSet.has(idx)) {
      ...
      if (ng >= safeCount) {
        ...
        const profit = w - wager; if (profit > 0 && onWin) onWin(profit);
  ...
  return (
    <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[70] p-4 anim-fade-in" onClick={onClose}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:'linear-gradient(180deg,#0a1828 0%,#030810 100%)',border:'2px solid rgba(6,182,212,.2)',borderRadius:24,maxWidth:880,width:'100%',overflow:'hidden',boxShadow:'0 0 60px rgba(6,182,212,.08),0 20px 60px rgba(0,0,0,.6)'}}>
        {/* HEADER */}
          ...
          <div style={{flex:1,padding:'18px 24px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:7,width:'100%',maxWidth:400,animation:shakeGrid?'mGridShake .5s ease-out':'none'}}>
              {Array.from({length:TOTAL},(_,idx)=>{
                ...
                else{bg='radial-gradient(circle,#064e3b,#022c22)';bdr=`2px solid rgba(34,197,94,${isFlash?.5:.2})`;shd=isFlash?'0 0 25px rgba(34,197,94,.35),inset 0 0 10px rgba(34,197,94,.1)':'0 0 8px rgba(34,197,94,.08)';anim='mTileGem .35s ease-out';}
                return(
                  <div key={idx} onClick={()=>canClick&&revealTile(idx)} className={`${!isRev?'m-tile-h':''}${!canClick&&!isRev?' m-tile-l':''}`}
                    ...
                    {isRev&&isM&&!isExp&&<span style={{opacity:.4,fontSize:20}}>💣</span>}
                    {!isRev&&gameOver&&!isM&&<span style={{opacity:.08,fontSize:16}}>💎</span>}
                    {!isRev&&phase==='setup'&&<span style={{opacity:.06,fontSize:15}}>?</span>}
                  </div>
                );
              })}
            </div>
...
export default function GamificationPlatform() {
  ...
  const [user, setUser] = useState({
    ...
    dailyClaimed: false,
    referrals: 0,
    gamePlays: { wheel: 3, scratch: 5, dice: 5, memory: 3, highlow: 5, katouch: 99, plinko: 5, tapfrenzy: 5, stopclock: 5, treasure: 3, mines: 5 },
  });
